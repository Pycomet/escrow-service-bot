...
def send_usdt_ethereum_transaction(
    sender_private_key, recipient_address, amount_usdt, log_file=None
):
    """
    ...
    try:
        ...
        if infura_id and infura_id != "your-infura-project-id":
            ...
            logger.warning(
                "Using public RPC endpoint. Consider setting INFURA_PROJECT_ID for better reliability."
            )
        ...
        logger.info(
            f"Sending {amount_usdt} USDT from {sender_address} to {recipient_address}"
        )
        ...
        if usdt_balance < amount_wei:
            logger.error(
                f"Insufficient USDT balance. Has {usdt_balance_readable}, need {amount_usdt}"
            )
            return None
        ...
        if eth_balance < min_eth_needed:
            ...
            logger.error(
                f"Insufficient ETH for gas. Has {eth_balance_readable}, need at least {min_eth_readable}"
            )
            return None
        ...
        logger.info(
            f"Transaction built - Gas: {estimated_gas}, Gas Price: {web3.from_wei(gas_price, 'gwei')} Gwei"
        )
...
def send_transaction(
    sender_private_key, recipient_address, amount, sender_address, trade_details
):
    """
