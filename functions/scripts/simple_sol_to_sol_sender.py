...
def send_transaction(
    ...
    try:
        ...
        # Escrow Wallet
        private_key_base58 = base58.b58encode(bytes.fromhex(hex_private_key)).decode(
            "utf-8"
        )
        keypair = Keypair.from_private_key(private_key_base58)
        # Fee Wallet
        fee_wallet_key_base58 = base58.b58encode(
            bytes.fromhex(fee_payer_private_key)
        ).decode("utf-8")
        ...
        if send_amount <= 0:
            raise ValueError(
                "The amount to send is too low to cover the transaction fee."
            )
        ...
        if tradeDetails["broker_fee"] > 0:
            broker_acc_initialized = is_account_initialized(
                client, PublicKey(brokerAddress)
            )
            a = (
                broker_fee
                if broker_acc_initialized
                else max(broker_fee, RENT_EXEMPT_BALANCE)
            )
        ...
        b = (
            send_amount
            if broker_acc_initialized
            else min((send_amount + broker_fee - RENT_EXEMPT_BALANCE), send_amount)
        )
