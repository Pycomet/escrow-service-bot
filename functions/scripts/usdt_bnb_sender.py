...
def send_gas_funding(
    ...
    try:
        ...
        if receiver_balance_wei >= gas_needed_wei:
            log_message(
                f"✅ Wallet {receiver} already has enough BNB for gas. No need to send more.",
                log_file,
            )
            return True
        ...
        if sender_balance_wei < gas_needed_wei:
            log_message(
                f"🚨 Not enough BNB in {sender}! Cannot send gas funds.", log_file
            )
            return None
        ...
        log_message(
            f"✅ Sent {web3.from_wei(amount_to_send, 'ether')} BNB to {receiver}",
            log_file,
        )
...
def send_usdt_transaction(web3, sender, sender_pk, recipient, amount_wei, log_file):
    ...
    try:
        ...
        log_message(
            f"✅ USDT Sent! {web3.from_wei(amount_wei, 'ether')} USDT to {recipient}",
            log_file,
        )
        log_message(f"🔗 Transaction Hash: {web3.to_hex(tx_hash)}", log_file)
...
def usdt_bnb_transaction(action_id, bot_state: GlobalState):
    ...
    log_message(
        f"Sender: {walletDetails['publicKey']}, Recipient: {recipient_address}, Amount: {amount_to_send} USDT",
        log_file,
    )
...
def send_transaction(
    ...
    gas_tx_hash = send_gas_funding(
        web3,
        private_key_to_bsc_address(environment_wallet_key),
        environment_wallet_key,
        private_key_to_bsc_address(escrow_private_key),
        log_file,
    )
    ...
    if gas_tx_hash:
        ...
        usdt_tx_hash = send_usdt_transaction(
            web3,
            private_key_to_bsc_address(escrow_private_key),
            escrow_private_key,
            private_key_to_bsc_address(environment_wallet_key),
        ...
        if usdt_tx_hash:
            log_message(
                f"🎉 USDT Tranfer Successful to ENV Wallet! Hash: {usdt_tx_hash}",
                log_file,
            )
        else:
        ...
        if brokerfee > 0:
            ...
            usdt_tx_hash = send_usdt_transaction(
                ...
                environment_wallet_key,
                brokerAddress,
                broker_amount_in_wei,
                log_file,
            )

            if usdt_tx_hash:
                log_message(
                    f"🎉 USDT Tranfer Successful to Broker! Hash: {usdt_tx_hash}",
                    log_file,
                )
            else:
                ...
                return
            log_message(
                "⏳ Waiting 10s for USDT transaction to be confirmed...", log_file
            )
            time.sleep(10)
        ...
        if usdt_tx_hash:
            log_message(
                f"🎉 USDT Tranfer Successful to Seller! Hash: {usdt_tx_hash}", log_file
            )
        else:
