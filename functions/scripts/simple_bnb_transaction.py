...
def send_bnb(sender, sender_pk, recipients, amounts, log_file, empty_wallet=False, max_gas_price_gwei=1, retries=3, delay=5):
    ...
    for i, recipient in enumerate(recipients):
        if empty_wallet and i == len(recipients) - 1:
            ...
            if amount_wei <= 0:
                log_message(f"❌ Not enough balance to empty wallet to {recipient}", log_file)
                return
        else:
        ...
        for attempt in range(retries):
            try:
                ...
                log_message(f"✅ Sent {web3.from_wei(amount_wei, 'ether')} BNB to {recipient}! TX: {web3.to_hex(tx_hash)}", log_file)
                time.sleep(5)  # Short delay
                break  

            except Exception as e:
                log_message(f"⚠️ Attempt {attempt + 1}/{retries} failed: {str(e)}", log_file)
                if attempt < retries - 1:
                    log_message(f"🔄 Retrying in {delay} seconds...", log_file)
                    time.sleep(delay)
                else:
                    log_message(f"❌ Failed to send BNB to {recipient}, exiting function.", log_file)
                    return  
...
def send_bnb_transaction(tradeId: str, bot_state: GlobalState):
    ...
    send_bnb(private_key_to_bsc_address(secret_key), secret_key, recepients, amounts, log_file)
