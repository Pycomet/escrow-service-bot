...
def send_doge_transaction(
    doge_address,
    doge_private_key_hex,
    recipient_address,
    amount_to_send,
    api_token,
    ...
    try:
        ...
        address_details = get_address_full(
            address=doge_address, coin_symbol="doge", api_key=api_token
        )
        utxos = []
        ...
        if Decimal(tradeDetails["fee"]) > 0:
            outputs.append(
                {"address": escrow_fee_wallet_address, "value": escrow_fee_satoshi}
            )

        if Decimal(tradeDetails["broker_fee"]) > 0:
            outputs.append(
                {"address": tradeDetails["brokerAddress"], "value": broker_fee_satoshi}
            )
        ...
        response = broadcast_signed_transaction(
