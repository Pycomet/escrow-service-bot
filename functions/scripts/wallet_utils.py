...
def generateWallet(action_id: str, bot_state: GlobalState):
    ...
    if tradeDetails["currency"] == "LTC":
        ...
        bot_state.save_wallet_info(
            action_id,
            wallet["mnemonic"],
            wallet["private_key"],
            wallet["bech32_address"],
            "LTC",
            inspect.getmodule(caller_frame[0]).__name__.split(".")[1],
        )
        return wallet["bech32_address"]
    elif (
        tradeDetails["currency"] == "SOL (Solana)"
        or tradeDetails["currency"] == "USDT (Solana)"
    ):
        ...
        or tradeDetails["currency"] == "USDT (BSC Bep-20)"
    ):
...
def sendtrans(bot_state: GlobalState, action_id: str):
    ...
    if action_id.startswith("TRADE"):
        ...
        tradeDetails = bot_state.get_tx_var(action_id)
        tradeDetails["sellerAddress"] = bot_state.get_item_details(
            tradeDetails["item_id"]
        )["sellerAddress"]
