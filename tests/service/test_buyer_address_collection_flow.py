...
class TestBuyerAddressCollectionFlow:
    ...
    @pytest.mark.asyncio
    async def test_complete_payment_to_address_flow(self, mock_seller_update, mock_buyer_update, mock_context):
        ...
        with patch("handlers.trade_flows.fiat.TradeClient.get_trade") as mock_get_trade, \
             patch("handlers.trade_flows.fiat.TradeClient.approve_fiat_payment") as mock_approve, \
             patch("handlers.trade_flows.fiat.TradeClient.request_buyer_address") as mock_request_address, \
             patch("handlers.join.db") as mock_db, \
            ...
            await CryptoFiatFlow.handle_seller_payment_review(mock_seller_update, mock_context)
    ...
    @pytest.mark.asyncio
    async def test_payment_status_monitoring_throughout_flow(self, mock_buyer_update, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_address_collection(self, mock_buyer_update, mock_context):
        ...
        with patch("handlers.join.db") as mock_db:
            ...
            assert ("Invalid address format" in args[0] or "Invalid USDT address format" in args[0])
    ...
    async def test_database_error_fallback(self, mock_buyer_update, mock_context):
        ...
        with patch("handlers.join.db") as mock_db, \
             patch("handlers.join.TradeClient.get_trade") as mock_get_trade, \
             patch("handlers.join.UserClient.get_user") as mock_get_user:
            ...
            args, _ = mock_buyer_update.message.reply_text.call_args
            assert ("Trade Details" in args[0] or "USDT" in args[0] or "normal_trade_123" in args[0])  # Trade info shown 