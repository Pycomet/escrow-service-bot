...
from telegram import Update, User, Message, Chat, CallbackQuery, InlineKeyboardMarkup, InlineKeyboardButton
...
def mock_update_fiat():
    ...
    update.callback_query.message = update.message # Link query's message to update's message
...
async def test_start_initial_setup(mock_update_fiat, mock_context_fiat):
    ...
    assert kwargs['reply_markup'].inline_keyboard[0][0].callback_data == "cancel_creation"

    assert context.user_data['trade_creation']['current_flow_step'] == AWAITING_AMOUNT
    assert context.user_data['trade_creation']['active_flow_module'] == CryptoFiatFlow.FLOW_NAME
...
async def test_handle_amount_input_valid(mock_update_fiat, mock_context_fiat):
    ...
    with patch('handlers.trade_flows.fiat.currency_menu', return_value=AsyncMock(spec=InlineKeyboardMarkup)) as mock_currency_menu:
        ...
        assert context.user_data['trade_creation']['current_flow_step'] == AWAITING_CURRENCY
...
async def test_handle_amount_input_invalid_value(mock_update_fiat, mock_context_fiat):
    ...
    assert context.user_data['trade_creation'].get('amount') is None # Amount should not be set
    assert context.user_data['trade_creation']['current_flow_step'] == AWAITING_AMOUNT # Should remain in amount step
