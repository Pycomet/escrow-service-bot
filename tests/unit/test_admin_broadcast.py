...
class TestAdminBroadcastManager:
    ...
    async def test_send_broadcast_message_success(self):
        ...
        mock_bot.send_message.assert_any_call(
            chat_id="123", text=ANY, parse_mode="HTML"
        )
        mock_bot.send_message.assert_any_call(
            chat_id="456", text=ANY, parse_mode="HTML"
        )
...
class TestAdminBroadcastHandlers:
    ...
    async def test_admin_broadcast_confirm_handler_success(
        ...
        with patch.object(
            AdminBroadcastManager, "send_broadcast_message", return_value=mock_result
        ):
    ...
    @pytest.mark.asyncio
    async def test_admin_broadcast_confirm_handler_no_data(
        self, mock_update_callback, mock_context
    ):
