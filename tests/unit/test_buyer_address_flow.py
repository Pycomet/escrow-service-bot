...
class TestBuyerAddressInput:
    ...
    @pytest.mark.asyncio
    async def test_valid_usdt_address_input(
        self, mock_buyer_update, mock_context, mock_trade
    ):
        ...
        with patch("handlers.join.db") as mock_db, patch(
            ...
            "handlers.join.TradeClient.calculate_trade_fee"
        ) as mock_fee:
    ...
    @pytest.mark.asyncio
    async def test_invalid_address_input(
        self, mock_buyer_update, mock_context, mock_trade
    ):
        ...
        with patch("handlers.join.db") as mock_db:
            ...
            assert (
                ...
                or "error occurred" in args[0].lower()
            )
...
class TestPaymentStatusCallback:
    ...
    @pytest.mark.asyncio
    async def test_payment_status_waiting_for_address(
    ...
    @pytest.mark.asyncio
    async def test_payment_status_trade_not_found(
        self, mock_buyer_update, mock_context
    ):
...
class TestAddressValidation:
    ...
    def test_valid_ethereum_addresses(self):
        ...
        for address in valid_addresses:
            ...
            assert (
                _validate_crypto_address(address, "USDT") == True
            )  # USDT can use ETH addresses
    ...
    def test_address_length_validation(self):
        ...
        assert (
            _validate_crypto_address(
                "0x742d35cc6e3f4dc5bf5b123456789abcdef12345", "ETH"
            )
            == True
        )
        ...
        assert (
            _validate_crypto_address("a_valid_length_address_here", "UNKNOWN") == True
        )
