...
from database.types import CoinAddressType, WalletTransactionType, WalletType
...
class TestWalletManager:
    ...
    def test_wallet_manager_initialization(self):
        ...
        assert (
            len(wallet_manager.DEFAULT_COINS) >= 5
        )  # Should have at least BTC, ETH, LTC, DOGE, SOL
    ...
    def test_create_wallet_for_user_success(self, mock_db, sample_user_id):
        ...
        mock_db.coin_addresses.insert_many.return_value = Mock(
            inserted_ids=["addr1", "addr2", "addr3"]
        )
    ...
    @patch("functions.wallet.db")
    def test_create_wallet_for_user_existing_wallet(
        self, mock_db, sample_user_id, sample_wallet
    ):
    ...
    def test_get_user_wallet(self, mock_db, sample_user_id, sample_wallet):
        ...
        # Assertions
        assert wallet == sample_wallet
        mock_db.wallets.find_one.assert_called_once_with(
            {"user_id": sample_user_id, "is_active": True}
        )
    ...
    def test_get_wallet_coin_address(self, mock_db, sample_coin_addresses):
        ...
        mock_db.coin_addresses.find_one.assert_called_once_with(
            {"wallet_id": "wallet_123", "coin_symbol": "BTC"}
        )
    ...
    async def test_refresh_wallet_balances(self, mock_db, sample_coin_addresses):
        ...
        assert mock_db.coin_addresses.update_one.call_count == len(
            sample_coin_addresses
        )
    ...
    def test_add_coin_to_wallet_success(self, mock_db, sample_wallet):
        ...
        with patch.object(WalletManager, "_decrypt_data") as mock_decrypt, patch.object(
            WalletManager, "_create_coin_address"
        ) as mock_create:
...
class TestWalletHandlers:
    ...
    @pytest.mark.asyncio
    async def test_wallet_handler_no_wallet(
        self, mock_send, mock_get_wallet, mock_update, mock_context
    ):
        ...
        # Set up callback query properly
        ...
        # Should call edit_message_text with wallet creation prompt
        mock_update.callback_query.edit_message_text.assert_called_once()
        call_args = mock_update.callback_query.edit_message_text.call_args
        message_text = call_args[0][0]
        assert "You don't have a wallet yet" in message_text
        assert "Welcome to Wallet Management" in message_text

    @patch("handlers.wallet.WalletManager.get_user_wallet")
    @patch("handlers.wallet.WalletManager.get_wallet_coin_addresses")
    @patch("handlers.wallet.send_message_or_edit")
    @pytest.mark.asyncio
    async def test_wallet_handler_with_wallet(
        self,
        mock_send,
        mock_get_addresses,
        mock_get_wallet,
        ...
        call_args = mock_update.callback_query.edit_message_text.call_args
        message_text = call_args[0][0]
        assert "My Wallet Overview" in message_text
    ...
    @patch("handlers.wallet.WalletManager.get_wallet_coin_addresses")
    @pytest.mark.asyncio
    async def test_wallet_create_handler_success(
        self,
        mock_get_addresses,
        mock_create_wallet,
        ...
        assert (
            mock_callback_update.callback_query.edit_message_text.call_count >= 2
        )  # Loading + success

        # Check final success message
        final_call = (
            mock_callback_update.callback_query.edit_message_text.call_args_list[-1]
        )
    ...
    @patch("handlers.wallet.WalletManager.create_wallet_for_user")
    @pytest.mark.asyncio
    async def test_wallet_create_handler_failure(
        self, mock_create_wallet, mock_callback_update, mock_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_wallet_refresh_handler_success(
        self, mock_wallet_manager_class, mock_callback_update, mock_context
    ):
    ...
    @patch("handlers.wallet.WalletManager.get_wallet_coin_addresses")
    @pytest.mark.asyncio
    async def test_wallet_details_handler_success(
        self,
        mock_get_addresses,
        mock_get_wallet,
        mock_callback_update,
...
if __name__ == "__main__":
    ...
    if results["test_session"]["status"] == "PASSED":
        ...
        print(
            "⚠️  Some tests failed. Check the detailed results file for more information."
        )
