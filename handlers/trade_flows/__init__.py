...
class TradeFlowHandler:
    @staticmethod
    async def initiate_flow_setup(update: Update, context: ContextTypes.DEFAULT_TYPE, trade_type: str) -> bool:
        ...
        if trade_type == TradeTypeEnums.CRYPTO_FIAT.value:
            ...
            logger.error(f"Invalid trade_type '{trade_type}' received in initiate_flow_setup.")
        ...
        if flow_class and hasattr(flow_class, 'start_initial_setup'):
            ...
            logger.warning(f"Flow class {flow_class} or method start_initial_setup not found for {trade_type}.")
    ...
    @staticmethod
    async def route_to_flow_method(update: Update, context: ContextTypes.DEFAULT_TYPE, trade_type: str, method_name: str) -> bool:
        ...
        if trade_type == TradeTypeEnums.CRYPTO_FIAT.value:
            ...
            logger.error(f"Invalid trade_type '{trade_type}' received in route_to_flow_method for method '{method_name}'.")
            return False
        ...
        if flow_class and hasattr(flow_class, method_name):
            ...
            logger.warning(f"Flow class {flow_class} or method {method_name} not found for {trade_type}.")
            return False
    ...
    async def route_trade_flow(update: Update, context: ContextTypes.DEFAULT_TYPE, trade_type: str):
        ...
        if trade_type == TradeTypeEnums.CRYPTO_FIAT.value:
            ...
            if target_message:
                ...
                logger.error("route_trade_flow: No message object found in update to send error.")
            return False