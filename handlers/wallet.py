...
from utils.keyboard import back_to_menu, wallet_details_menu, wallet_menu
...
async def wallet_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
                    ...
                    [
                        [
                            InlineKeyboardButton(
                                "➕ Create My Wallet", callback_data="wallet_create"
                            )
                        ],
                        [
                            InlineKeyboardButton(
        ...
        if coin_addresses:
            ...
            wallet_text += (
                f"{EmojiEnums.WARNING.value} <b>No coin addresses found</b>\n"
            )
        ...
        await query.edit_message_text(
            wallet_text, parse_mode="HTML", reply_markup=keyboard
        )
...
async def wallet_create_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text(Messages.wallet_creating(), parse_mode="HTML")
...
async def wallet_refresh_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Show loading message
        await query.edit_message_text(Messages.wallet_refreshing(), parse_mode="HTML")
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "🔄 Try Again",
                                callback_data=f"wallet_refresh_{wallet_id}",
                            )
                        ],
                        [
                            InlineKeyboardButton(
...
async def wallet_details_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text(
            details_text, parse_mode="HTML", reply_markup=keyboard
        )
    ...
    try:
        ...
        if coin_addresses:
            ...
            for coin_address in coin_addresses:
                ...
                balance_text += (
                    f"{status_icon} {coin_emoji} <b>{coin_symbol}:</b> {balance}\n"
                )
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            f"{EmojiEnums.REFRESH.value} Refresh Now",
                            callback_data=f"wallet_refresh_{wallet['_id']}",
                        ),
                        InlineKeyboardButton(
                            "📋 Details",
                            callback_data=f"wallet_details_{wallet['_id']}",
                        ),
                    ],
                    [
                        InlineKeyboardButton(
                            f"{EmojiEnums.BACK_ARROW.value} Back to Wallet",
                            callback_data=CallbackDataEnums.MY_WALLETS.value,
                        )
                    ],
                ]
...
async def wallet_refresh_general_handler(
...
async def wallet_transactions_handler(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    ...
    try:
        ...
        transactions_text += (
            f"💡 <b>Note:</b> Transaction history will appear here when you:\n"
        )
