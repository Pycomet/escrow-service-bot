...
async def delete_trade_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🗑 Select a trade to delete:", reply_markup=InlineKeyboardMarkup(keyboard)
    )


async def handle_delete_trade_callback(
    ...
    if data.startswith("delete_trade_"):
        ...
        if (
            trade["seller_id"] != query.from_user.id
            and trade["buyer_id"] != query.from_user.id
        ):
            await query.edit_message_text(
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "✅ Yes, Delete", callback_data=f"confirm_delete_{trade_id}"
                        )
                    ],
...
def register_handlers(application):
    ...
    application.add_handler(
        CallbackQueryHandler(
            handle_delete_trade_callback, pattern="^(delete_trade_|confirm_delete_)"
        )
    )
