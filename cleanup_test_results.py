...
def cleanup_test_results(keep_count=3, dry_run=False, remove_all=False):
    ...
    for pattern in patterns:
        ...
        files = [f for f in files if not any(exclude in f for exclude in exclude_patterns)]
        ...
        if files_to_remove:
            ...
            print(f"   Action: {'Would remove' if dry_run else 'Removing'} {len(files_to_remove)} files")
            ...
            for file_path in files_to_remove:
                ...
                if dry_run:
                    print(f"     - {file_path} ({size_kb:.1f}KB, {file_age.strftime('%Y-%m-%d %H:%M')})")
                else:
                    try:
                        os.remove(file_path)
...
Examples:
  ...
  %(prog)s --all              # Remove all result files
    ...
    parser.add_argument("--all", action="store_true",
                       help="Remove all test result files")
