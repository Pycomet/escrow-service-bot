...
class CommunityScheduler:
    ...
    def _load_schedule_config(self) -> Dict:
        ...
        return {
            ...
            "weekly_schedule": {
                ...
                "saturday": [{"time": "11:00", "type": "educational"}],
                "sunday": [{"time": "16:00", "type": "weekly_analysis"}],
            },
        }
    ...
    async def _setup_scheduled_jobs(self):
        ...
        try:
            ...
            for day_name, posts in weekly_schedule.items():
                for post_config in posts:
                    ...
                    if not post_time or not content_type:
                        logger.warning(
                            f"Invalid post config for {day_name}: {post_config}"
                        )
                        continue
            ...
            logger.info(
                f"Set up {len(self.scheduler.get_jobs())} scheduled community posts"
            )
    ...
    async def _generate_and_post_content(self, content_type: str):
        ...
        try:
            ...
            logger.error(
                f"Error in scheduled content generation for {content_type}: {e}"
            )
