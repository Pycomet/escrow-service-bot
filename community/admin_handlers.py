...
class CommunityAdminHandlers:
    ...
    @staticmethod
    async def community_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
📅 <b>Next Posts:</b>"""
        ...
        keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("📝 Post Now", callback_data="community_post_now"),
                InlineKeyboardButton("⚙️ Settings", callback_data="community_settings")
            ],
            [
                InlineKeyboardButton("📊 Statistics", callback_data="community_stats"),
                InlineKeyboardButton("🧪 Test System", callback_data="community_test")
            ],
            [
                InlineKeyboardButton("🔄 Restart Scheduler", callback_data="community_restart"),
                InlineKeyboardButton("📋 View Posts", callback_data="community_view_posts")
            ],
            [
        ...
        await query.edit_message_text(status_text, parse_mode='HTML', reply_markup=keyboard)
    ...
    async def post_now_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("📈 Market Brief", callback_data="post_now_market_brief"),
                InlineKeyboardButton("🎓 Educational", callback_data="post_now_educational")
            ],
            [
                InlineKeyboardButton("🔧 Platform Update", callback_data="post_now_platform_update"),
                InlineKeyboardButton("🔐 Security Tip", callback_data="post_now_security_tip")
            ],
            [
                InlineKeyboardButton("📊 Weekly Analysis", callback_data="post_now_weekly_analysis")
            ],
            [
    ...
    async def settings_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            posting_settings = db.community_settings.find_one({"type": "posting_settings"}) or {}
            schedule_settings = db.community_settings.find_one({"type": "schedule_config"}) or {}
            
            posting_enabled = posting_settings.get("enabled", True)
            posts_per_day = schedule_settings.get("schedule", {}).get("posts_per_day", 2)
...
🔧 <b>Actions:</b>"""
        ...
        keyboard = [
            ...
            [
                InlineKeyboardButton("📅 Edit Schedule", callback_data="community_edit_schedule"),
                InlineKeyboardButton("🔄 Reset to Default", callback_data="community_reset_schedule")
            ],
            [
                InlineKeyboardButton("🔙 Back", callback_data="admin_community")
            ]
    ...
    @staticmethod
    async def toggle_posting_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            # Get current status
            settings = db.community_settings.find_one({"type": "posting_settings"}) or {}
    ...
    async def test_system_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if channel_test.get("success"):
                ...
                test_results.append(f"   📍 Channel: {channel_test.get('channel_title', 'Unknown')}")
            else:
                test_results.append(f"❌ Channel Connection: {channel_test.get('error', 'Unknown error')}")
            ...
            keyboard = [
                [
                    InlineKeyboardButton("📤 Post Test Message", callback_data="community_post_test"),
                ],
                [
                    InlineKeyboardButton("🔙 Back", callback_data="admin_community")
                ]
    ...
    @staticmethod
    async def post_test_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
