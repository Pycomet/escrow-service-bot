...
async def test_market_data():
    ...
    try:
        ...
        logger.info(f"   📊 BTC: ${market_data['prices']['BTC']:,.0f} ({market_data['changes']['BTC']:+.1f}%)")
        logger.info(f"   📊 ETH: ${market_data['prices']['ETH']:,.0f} ({market_data['changes']['ETH']:+.1f}%)")
...
async def test_content_generation():
    ...
    try:
        ...
        content_types = ["educational", "market_brief", "platform_update", "security_tip"]
    ...
    try:
        ...
        if status.get('next_posts'):
            ...
            for post in status['next_posts'][:3]:
                logger.info(f"      • {post.get('content_type')}: {post.get('next_run', 'Unknown')}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--post", action="store_true", help="Actually post test message to channel")
    parser.add_argument("--samples", action="store_true", help="Generate sample content")
