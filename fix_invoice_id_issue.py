...
def fix_invoice_id_issue():
    ...
    try:
        ...
        logger.info(f"Found {len(empty_invoice_trades)} trades with empty string invoice_id")
        ...
        logger.info(f"✅ Updated {result.modified_count} trades from empty string to null")
        ...
        try:
            ...
            db.trades.delete_many({"_id": {"$in": ["TEST_TRADE_1", "TEST_TRADE_2", "TEST_TRADE_3", "TEST_TRADE_4"]}})
        ...
        logger.info(f"Trades with empty string invoice_id after fix: {len(empty_invoice_trades_after)}")
        logger.info(f"Trades with null invoice_id after fix: {len(null_invoice_trades_after)}")
